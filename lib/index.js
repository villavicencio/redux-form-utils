        ...
        value: function render() {
          ...
          return _react2.default.createElement(Component, _extends({}, this.props, { fields: fields.reduce(function (prev, curr) {
              ...
              if (typeof curr === 'string') {
                ...
                var key = curr.key,
                    _curr$valueKey = curr.valueKey,
                    valueKey = _curr$valueKey === undefined ? 'value' : _curr$valueKey,
                    _curr$changeType = curr.changeType,
                    changeType = _curr$changeType === undefined ? 'onChange' : _curr$changeType;
                var resolver = curr.resolver,
                    resovler = curr.resovler;
  ...
  return {
    state: {
      form: fields.reduce(function (prev, curr) {
        if (typeof curr === 'string') {
          ...
          var key = curr.key,
              _curr$valueKey2 = curr.valueKey,
              valueKey = _curr$valueKey2 === undefined ? 'value' : _curr$valueKey2,
              initValue = curr.initValue;
    ...
    reducer: function reducer(state, action) {
      ...
      switch (action.type) {
        ...
        case '@@form/CLEAR':
          {
            var _fieldConfig = findConfig(action.meta.field);

            return _extends({}, state, {
              form: _extends({}, state.form, _defineProperty({}, action.meta.field, _extends({}, state.form[action.meta.field], _defineProperty({}, '' + (_fieldConfig.valueKey || 'value'), _fieldConfig.initValue || ''))))
            });
          }
